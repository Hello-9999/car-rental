...
export const fetchModelData = async (dispatch) => {
  try {
    ...
    if (res.ok) {
      ...
      console.log(data["cars"], "dfa");

      const models = data["cars"].map((cur) => cur.model);
      console.log(models, "models");
      dispatch(setModelData(models));

      //getting comapnys from data
      const brand = data["cars"].map((cur) => cur.brand);
      ...
      const locations = data["locations"].map((cur) => cur.location);
      dispatch(setLocationData(locations));

      //getting districts from data
      const districts = data["locations"].map((cur) => cur.district);
      ...
      //setting whole data
      const wholeData = data["locations"];
...
const AddProductModal = () => {
  ...
  const onSubmit = async (addData) => {
    try {
      ...
      // formData.append("transmitionType", addData.transmitionType);
      ...
      const data = await fetch(
        `${import.meta.env.VITE_PRODUCTION_BACKEND_URL}/api/admin/addProduct`,
        {
          method: "POST",
